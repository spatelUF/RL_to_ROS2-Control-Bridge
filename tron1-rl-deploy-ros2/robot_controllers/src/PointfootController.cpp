...
namespace robot_controllers
{
  ...
  bool PointfootController::loadModel()
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("PointfootController"), "Failed to retrieve encoder path from the parameter server!");
      return false;
    }
    ...
    // Encoder session
    RCLCPP_INFO(rclcpp::get_logger("PointfootController"), "Loading encoder from: %s", encoderModelPath.c_str());
    ...
    for (size_t i = 0; i < encoderSessionPtr_->GetInputCount(); i++) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("PointfootController"), "GetInputName: %s", encoderSessionPtr_->GetInputName(i, allocator));
    ...
    for (size_t i = 0; i < encoderSessionPtr_->GetOutputCount(); i++) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("PointfootController"), "GetOutputName: %s", encoderSessionPtr_->GetOutputName(i, allocator));
